...
def get_geo_loss(gt_geo, pred_geo):
    ...
    w_inter = torch.min(d3_gt, d3_pred) + torch.min(d4_gt, d4_pred)
    h_inter = torch.min(d1_gt, d1_pred) + torch.min(d2_gt, d2_pred)
...
class Loss(nn.Module):
    ...
    def forward(self, gt_score, pred_score, gt_geo, pred_geo, ignored_map):
        ...
        classifiy_loss = get_dice_loss(gt_score, pred_score*(1-ignored_map))
        ...
        print("classify loss is {:.8f}, angle loss is {:.8f}, iou loss is {:.8f}".format(classifiy_loss, angle_loss, iou_loss))

        return geo_loss + classifiy_loss
